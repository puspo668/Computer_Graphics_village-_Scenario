...
GLfloat sunY = 1.0f;  // Start from top
GLfloat sunSpeed = -0.002f; // Moves downward
GLfloat moonY = 0.9f; // Starting position for the moon
GLfloat moonSpeed = -0.002f; // Speed for moon

GLfloat position3 = 1.0f;  // Starting position for rain
GLfloat speed3 = -0.02f;   // Rain falling speed

bool isRaining = false;    // Rain state
...
void drawSun() {
    glColor3ub(255, 0, 0); // Sun color (orange)
    glBegin(GL_POLYGON);
    for (int i = 0; i < 360; i++) {
        float angle = i * 3.14159f / 180;
        glVertex2f(0.1f * cos(angle), sunY + 0.1f * sin(angle)); // Circle for sun
    }
    glEnd();
}
void sunn(int value) {
    if (sunY > -1.0f) { // Move until it disappears
        sunY += sunSpeed;
    }
    if (sunY <= -0.1f) { // Moon becomes visible after sunset
        moonY = 0.9f; // Set moon's initial position
    }
    glutPostRedisplay();
    glutTimerFunc(50, sunn, 0); // Update animation every 50ms
}
...
// 🌧️ Draw Rain Drops
void drawRain() {
    glColor3ub(0, 191, 255); // Light blue for rain
    glBegin(GL_LINES);
    for (float i = -1.0f; i < 1.0f; i += 0.1f) { // Multiple raindrops
        glVertex2f(i, position3);
        glVertex2f(i, position3 - 0.1f);
    }
    glEnd();
}
...
void drawCloud(float x, float y) {
    glColor3ub(200, 200, 200); // Gray cloud color
    glBegin(GL_POLYGON);
    for (int i = 0; i < 360; i++) {
        float angle = i * 3.14159f / 180;
...
// ☁️☁️ Draw Multiple Clouds
void drawClouds() {
    drawCloud(-0.6f, 0.8f);
    drawCloud(-0.4f, 0.85f);
    drawCloud(0.0f, 0.9f);
    drawCloud(0.4f, 0.85f);
}
...
void moon() {
    int i;
    GLfloat xx = -0.5f; GLfloat yx = 0.93f; GLfloat radiusx = 0.06f;
    int triangleAmount = 20;
    GLfloat twicePi = 2.0f * 3.14159f;

    // Draw moon base
    glBegin(GL_TRIANGLE_FAN);
    glColor3ub(0, 51, 204); // Moon color (blue)
    ...
    // Draw moon shadow (crescent effect)
    GLfloat x = -0.5f; GLfloat y = 0.9f; GLfloat radius = 0.06f;
    glBegin(GL_TRIANGLE_FAN);
    glColor3ub(242, 242, 242); // Shadow color (white)
    glVertex2f(x, y); // center of circle
    for (i = 0; i <= triangleAmount; i++) {
        glVertex2f(
            x + (radius * cos(i * twicePi / triangleAmount)),
...
void drawFish(float x, float y) {
    glColor3ub(255, 165, 0); // Orange fish
    glBegin(GL_TRIANGLES);
    glVertex2f(x, y);
...
float fishX = -1.0f; // Starting position
void updateFish(int value) {
    fishX += 0.01f; // Move fish to the right
    if (fishX > 1.0f) fishX = -1.0f; // Reset when off screen
    glutPostRedisplay();
    glutTimerFunc(100, updateFish, 0); // Repeat
}
...
void river()
{
   ...
   glColor3ub(38, 154, 214);
    ...
    glVertex2f(-1.0f,-1.0f);
    glEnd();
     // Draw fish at different heights
    drawFish(fishX, 0.3f);
    drawFish(fishX + 0.2f, 0.1f);
    drawFish(fishX + 0.4f, -0.2f);
}
void hut()
{
    glBegin(GL_POLYGON);
...
void StartingText()
{
    ...
    for(int i = 0; text[i] != '\0'; i++)
    {
        ...
        sprintf(text,"  PUSHPO HASAN",5.00,8.00);
    glColor3f(0, 1, 0);
    glRasterPos2f( -100 , 0 );
    for(int i = 0; text[i] != '\0'; i++)
    {
...
void display() {
    ...
    sky();
     drawSun(); // Draw sun
    if (sunY <= -0.1f) { // Show the moon only after sunset
        moon();
    }
    //sun();
    glPushMatrix();
    ...
    backgroundtree();
    if (isRaining) {
        drawClouds(); // Clouds appear during rain
        drawRain();   // Show rain animation
    }
     drawSun();
    river();
    glPushMatrix();
